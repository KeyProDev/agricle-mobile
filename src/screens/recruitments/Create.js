...
import {create, getAll} from "../../redux/Recruitment/actions";
...
const Create = ({ navigation }) => {
    ...
    const [ recruitment, setRecruitment ] = useState({
        ...
        work_time_start: new Date(new Date().setHours(9,0,0)),
        work_time_end: new Date(new Date().setHours(17,0,0)),
        break_time: '',
        lunch_mode: 0,
        pay_mode: 'cash',
        traffic_type: 'beside',
    ...
    const createRecruitment = (status = 'draft') => {
        setIsLoading(true);
        ...
        if(recruitment.image.type && recruitment.image.uri) {
            data.append('image', {
        ...
        dispatch(create(data, navigation));
    }
            ...
            <PTRView onRefresh={refresh}>
                <ScrollView style={styles.container}>
                    <View row style={styles.imageArea}>
                        <Image source={recruitment.image == '' ? emptyImage : recruitment.image} style={{ width: width, height: width * 0.6, resizeMode: 'stretch', }}/>
                        <Button
                            onlyIcon
                            icon="camera"
                    ...
                    <View style={styles.formArea}>
                        ...
                        <RadioGroup row marginT-s2 initialValue={recruitment.traffic_type} onValueChange={handleRecruitmentChange('traffic_type')}>
                            <Text $textDefault color={Colors.cyan10}>
                        ...
                        {
                            viewPreTags && preClothes.filter(item => recruitment.clothes.indexOf(item) === -1).map((item, index) => (
                        ...
                        />
                        <View row center>
                            <PaperButton icon={'content-copy'} mode={'contained'} color={Colors.blue30} onPress={() => createRecruitment(true)} disabled={isLoading}>
                                {t['action']['publish']}
                            </PaperButton>
                            <PaperButton icon={'content-save'} mode={'contained'} color={Colors.green30} onPress={() => createRecruitment()} style={{ marginLeft: 5 }} disabled={isLoading}>
                                {t['action']['draft']}
                            </PaperButton>
                        </View>
                    </View>
